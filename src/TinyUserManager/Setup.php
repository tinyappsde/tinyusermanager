...
class Setup {
	...
	public static function createUserTable(): bool {
		...
		$stmt = $db->prepare(str_replace('__PREFIX__', Database::getPrefix(), file_get_contents(__DIR__ . '/sql/tiny_users.sql')));
	...
	public static function createConfirmationTable(): bool {
		...
		$stmt = $db->prepare(str_replace('__PREFIX__', Database::getPrefix(), file_get_contents(__DIR__ . '/sql/tiny_confirmation_tokens.sql')));
		return $stmt && $stmt->execute();
	}
	...
	public static function createPasswordForgotTable(): bool {
		...
		$stmt = $db->prepare(str_replace('__PREFIX__', Database::getPrefix(), file_get_contents(__DIR__ . '/sql/tiny_password_forgot_tokens.sql')));
		return $stmt && $stmt->execute();
	}
}
