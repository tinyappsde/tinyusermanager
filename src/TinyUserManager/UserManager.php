...
class UserManager {
	...
	public static function findUser(string $email): ?User {
		...
		$stmt = $db->prepare('SELECT * FROM `' . Database::getPrefix() . 'users` WHERE `email`=:email');
	...
	public static function getUser(int $id): ?User {
		...
		$stmt = $db->prepare('SELECT * FROM `' . Database::getPrefix() . 'users` WHERE `id`=:id');
		$stmt->execute([':id' => $id]);
	...
	public static function updateUser(User $user): bool {
		...
		if ($stmt = $db->prepare('UPDATE `' . Database::getPrefix() . 'users` SET `email`=:email' . $computedSetPreparation . ' WHERE `id`=:id')) {
	...
	public static function createUser(string $email, string $password, ?array $fields = null, $skipConfirmation = false): ?User {
		...
		try {
			...
			if (!$stmt = $db->prepare('INSERT INTO `' . Database::getPrefix() . 'users` SET `email`=:email, `password`=:password' . $fieldsQueryPart)) {
