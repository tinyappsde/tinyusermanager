...
class ConfirmationHandler {
	...
	public static function sendConfirmationEmail(User $user, EmailConfig $emailConfig, string $emailSubject, string $emailTemplate): bool {
		...
		$stmt = $db->prepare('INSERT INTO `' . Database::getPrefix() . 'confirmation_tokens` SET `user_id`=:id, `token`=:token ON DUPLICATE KEY UPDATE `token`=:tokenupdate');
	...
	public static function confirmUser(User $user, string $token): bool {
		...
		$stmt = $db->prepare('SELECT * FROM `' . Database::getPrefix() . 'confirmation_tokens` WHERE `user_id`=:id');
		$stmt->execute([':id' => $user->getId()]);

		if ($result = $stmt->fetchObject()) {
			if (password_verify($token, $result->token)) {
				$stmt2 = $db->prepare('UPDATE `' . Database::getPrefix() . 'users` SET `confirmed`=true WHERE `id`=:id');
				$stmt2->execute([':id' => $user->getId()]);
				$stmt3 = $db->prepare('DELETE FROM `' . Database::getPrefix() . 'confirmation_tokens` WHERE `user_id`=:id');
